...
\begin{example}
	The following \define{concrete categories}\footnote{These are categories which
  we can fruitfully consider to have as objects sets with ``additional
  structure'' and certain functions of the underlying sets as arrows.} are
  familiar to the student of mathematics:
  \begin{itemize}
    \itemsep-0.2em
...
\begin{example}
  The following arise less outside of category theory itself:
  \begin{itemize}
    \itemsep-0.2em
...
\begin{equation*}
  g ·^\op h \defeq h · g
\end{equation*}
Now, suppose we know that $(A,≤)$ has all finite meets. Then we immediately know
that $(A,≤')$ has all finite joins. In general, if we know some theorem $T$
about $(A,≤)$ or $(G,·,e)$. It seems like there should be a purely mechanical
process via which we can discover an appropritate ``opposite theorem'' that
applies to $(A,≤^\op)$ or $(G,·^\op,e)$.
...
Usefully, isomorphisms are self-dual. This means that isomorphic objects in
$\bfC$ are isomorphic in $\bfC^\op$ and vice-versa.

If $C$ is a concept defined in the language of categories, there is a dual
definition $C^\op$, obtained in the same way as $S^\op$ above. From now on, dual
concepts and statements will be introduced in pairs, and typeset like so:
...
\begin{definition}\label[definition]{def:cone-cocone}
  ...
  \vspace{-0.3em}\dual{
    ...
    $\braces{i_{v}:\apply{D}{v}→C}_{v∈G_0}$ such that the following triangle
    commutes for all $u,v∈G_0$ and $e∈G_1(u,v)$:
    \begin{center}
      \begin{tikzcd}[row sep=large, column sep=small, ampersand replacement=\&]
...
\begin{definition}\label[definition]{def:complete-and-cocomplete}
  A category $\bfC$ \define{has (co)limits of shape $G$} if any diagram of shape
...
\cref{chap:category-theory}, but their specific constructions will be important
...
\begin{lemma}\label[lemma]{lemma:limit-prop}
  The property of being a limit cone is a proposition. Explicitly, for any
  category $\bfC$, graph $G$, diagram $D$ of shape $G$ in $\bfC$,
  and object $X:\bfC$,
  \begin{equation*}
    \apply{\isProp}
...
We construct M-types as certain limits in $\universe$. The goal of this section
is to prove that these exist.
...
\begin{lemma}[\coqname{Induction.M.Limits.homot\_cone\_weq\_cone}]
