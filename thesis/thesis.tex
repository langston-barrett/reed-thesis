...
\begin{sidewaystable}
  ...
  \end{tabular}
  \caption{\label{fig:symbols}Symbols and their interpretations}
\end{sidewaystable}
...
\begin{gatherjot}
  ...
  %    {∃ v.\mvar{a}}
\end{gatherjot}
For the sake of clarity, ``$Γ$'', ``$⊢$'', and the horizontal line
``$\frac{\hspace{1em}}{\hspace{1em}}$'' are symbols in our metalanguage
(English), whereas ``$⊤,⊥,¬,→,∧,∨$'' are symbols in the object language. For
example $(\mvar{a}∧\mvar{b})∨\mvar{c}$ is a valid expression of our object
language whereas $(Γ ⊢ \prop{\mvar{a}})∨\mvar{b}$ is incoherent.
