...
in $\universe$. The next few lemmas explore some particular examples
before defining and constructing general limits.

\begin{lemma}\label[lemma]{lemma:universe-binproducts}
...
While there are more direct proofs of this lemma, the following is based on the
observation made in \cref{rmk:prod-coprod-universal-adj}, and will carry over to
the case of general limits. We begin with another lemma:
...
\begin{proof}
  ...
  \begin{align*}
    \begin{split}
      f : &(C→A×B) → \\
          &(C→A)×(C→B) \\
      \apply{f}{h} &\defeq (\pr{1}∘\apply{h},\pr{2}∘\apply{h})
    \end{split}
    \begin{split}
      g : &(C→A)×(C→B) → \\
          &(C→A×B) \\
  ...
  \begin{align*}
    \apply{(f∘g)}{(h,i)}
    &\≡ \apply{f}{(\λ{c}{(\apply{h}{c},\apply{i}{c})})} \\
  ...
  and the η-rule for the product type \TODO{reference}.
\end{proof}
...
\begin{lemma}
  ...
  \begin{gather*}
    \ttfun{postcomp}:\∏{d:\bfC}{\∏{f:d\catarrow c}{
      (d \catarrow a)×(d \catarrow b)
    }} \\
    \appply{d}{f} \defeq (p_1∘f,p_2∘f)
  \end{gather*}
  If $\ttfun{postcomp}$ is an equivalence, $c$ is a product of $a$ and $b$.
\end{lemma}
\begin{proof}
	TODO: define hfiber, etc
\end{proof}
