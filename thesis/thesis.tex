...
%     \abbreviation{FOL}  	&  Classical first-order logic \\
%     \abbreviation{IPL}  	&  Intuitionistic propositional logic \\
%     \abbreviation{LC}  	  &  Church's untyped λ-calculus \\
%     \abbreviation{STLC}  	&  Simply-typed λ-calculus \\
%     \abbreviation{TLC}  	&  Typed λ-calculus
%   \end{tabular}
% \end{table}
...
will introduce Church's λ-calculus \abbreviation{LC} (and the simply-typed and
typed versions \abbreviation{STLC} and \abbreviation{TLC}). Finally, in
\cref{sec:curry-howard}, we will discuss the fundamental and
...
including the λ-calculus (\abbreviation{LC}), Turing machines, Post systems,
...
general.\footnote{A comprehensive study of \abbreviation{LC} is beyond the scope
  of this thesis. For a reference on \abbreviation{LC} and \abbreviation{STLC},
  see \cite{barendregt}.}

The type theory considered in \cref{chap:type-theory} is an extension of the
typed λ-calculus (\abbreviation{TLC}) of \cref{subsec:more-complex-types}, which
itself an extension of the simply-typed λ-calculus (\abbreviation{STLC}) of
...
rule\index{Computation rules!In \abbreviation{LC}} and subject to the following
unicity rule:
\begin{align*}
  {\prftree[r]{\footnotesize β}
    {Γ ⊢ \term{\mvar{a}}}
...
\begin{theorem}[Church-Rosser]\label[theorem]{thm:church-rosser}
  The order in which we apply computation rules to a term of \abbreviation{LC}
...
\begin{definition}\label[definition]{def:normal-form}
	If $\mvar{a}$ is a term of \abbreviation{LC} such that there does not exist
...
The use of the definite article in the above definition is justified by
...
\define{types}\index{Types!Simply-typed λ-calculus} to \abbreviation{LC} to get
\abbreviation{STLC}, the simply-typed λ-calculus. We consider two new judgments
...
rule\index{Introduction rules!In \abbreviation{STLC}} tells us how to form
...
\end{align*}
The computation rules\index{Computation rules!In \abbreviation{STLC}} and
...
\begin{theorem}\label[theorem]{thm:strong-normalization}
  \abbreviation{STLC} is \define{strongly normalizing}; every term has exactly
  one normal form.
\end{theorem}
...
The ontology of \abbreviation{STLC} is severely limited: it can only express
...
from simpler ones (e.g.\ $→$ from \abbreviation{STLC}). In
...
following rules\index{Formation rules!In \abbreviation{TLC}}: 
\begin{gatherjot}
  \prftree[r]{}{}{Γ⊢\type{⊤}}
  \qquad
  \prftree[r]{}{}{Γ⊢\type{⊥}}
...
The elimination rules\index{Elimination rules!In \abbreviation{TLC}}
...
the computation and unicity rules are identical to those in \abbreviation{IPL}
...
for describing more complicated data to \abbreviation{STLC} looked not merely
analogous, but \textit{identical to} \abbreviation{IPL} (again, modulo simple
notational changes).
...
could be interpreted as valid propositions in \abbreviation{IPL}. This
observation squared nicely with what's called the Brouwer-Heyting-Kolmogorov
interpretation of \abbreviation{IPL}, which asserts that the semantics thereof
...
ways.\footnote{My own favorite is that it is not really a correspondence nor an
  isomorphism, but indeed should be read as an \textit{identity}, an expression
  of the fundamental sameness of logic and computation.}
It is sometimes read as asserting
that proofs of \abbreviation{IPL} can be encoded into \abbreviation{TLC}. This
translation has the nice property that type-checking (the process of determining
whether or not has a given type in a given context $Γ$) is decidable: it can be
performed mechanically, preferably by a computer. This gives such proofs a
strong epistemological footing.

\chapter{Type theory}
\label{chap:type-theory}

Whatever the meaning of the Curry-Howard correspondence, its utility is
circumscribed by the fact that \abbreviation{IPL}/\abbreviation{TLC} as
presented do not match the expressive power of classical \abbreviation{FOL}:
...
The system described in this section enriches \abbreviation{TLC} with
...
resulting system (univalent type theory, or \abbreviation{UTT}) has similar
expressive power to \abbreviation{ZFC}+\abbreviation{FOL}.\footnote{Indeed, the
...
\abbreviation{UTT} corresponds closely to $\caH$ in
...
power of homotopy type theory (\abbreviation{HoTT}), specifically we leave out
...
\begin{remark}\label[remark]{rmk:lambek}
	We now have the vocabulary to extend the correspondence of
  \cref{sec:curry-howard} to statements about a certain kind of category.
  \TODO{}
\end{remark}
...
\begin{definition}[\unimathname{W.Fibered.fibered\_alg},
  ...
  Inspired by the homotopical semantics for \abbreviation{UTT},
