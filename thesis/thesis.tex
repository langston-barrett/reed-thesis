...
This proof appeared in the \Agda{} formalization of\cite{non-wellfounded},
this is its first appearance ``de-formalized''.
...
\begin{proof}
  ...
  remains to show
  \begin{equation*}
    \propeq{(Y\to PY)}{\transport{Z\mapsto PZ}{\apply{\ua}{q}}{\alpha}}{\beta}.
  \end{equation*}
  By function extensionality (\cref{thm:funext}), it suffices
  to show that they are homotopic, that is, propositionally equal on all inputs. 
  ...
  \end{align*}
  Note that the last step utilizes the idea of \textit{proof-relevant
    mathematics}. Although we define $q$ within a proof, we can (consistently,
  without cheating) refer to its definition from another proof. Also note the
  use of \cref{notation:weq-coerce}.
\end{proof}
