...
\begin{definition}\label[definition]{def:jdeq-ipl}
  ...
  $\mvar{q}[\mvar{p}\coloneqq\mvar{r}]$).\footnote{
    ...
    to $\intro{⊥}$), there's no corresponding β-rule.}
\end{definition}
...
By the computation rules, this term obeys the following identities (in fact,
this is just a rewrite of the β-rules using the composition we just defined):
\begin{align*}
...
% \begin{tikzcd}[sep=large]
%   {} & C\arrow[ddl, bend right, swap, "f"{name=F}]\arrow[ddr, bend left, "g"{name=G}]
%         \arrow[d,dashed, bend right, swap, "\mathsf{rec}"{name=FG1}]
%         \arrow[d,dashed, bend left, ""{name=FG2}] & {} \\
%   {} & A\times B \arrow[dl, "\mathsf{proj}_1"{name=P1}]
%                  \arrow[dr,swap, "\mathsf{proj}_2"{name=P2}] & {} \\
%   A & {} & B
%   % filling in the cells
...
\begin{remark}\label[remark]{rmk:universal-algebra}
  ...
  coproducts, an algebra for an functor of the form $A↦(A×A)+A+1$ consists of:
  \begin{itemize}
    \itemsep0em
