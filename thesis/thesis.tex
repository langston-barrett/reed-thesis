...
\begin{definition}\label[definition]{def:limit-and-colimit}
  Given \ldots,
  \vspace{-0.3em}\dual{
    a \define{limit}\index{Limit} of \ldots
    consists of \ldots satisfying the following universal property:

    For any other \ldots, there is a unique arrow \ldots making the following
    diagram commute:
    % \begin{center}
    %   \begin{tikzcd}[sep=large,ampersand replacement=\&]
    %     {} \& D\arrow[dl,swap,"q_1"]\arrow[dr,"q_2"]
    ...
    a \define{pushout}\index{Pushout} of \ldots
    consists of \ldots satisfying the following universal property:

    For any other ``candidate pushout'' \ldots, there is a unique arrow
    \ldots making the following diagram commute:
    % \begin{center}
    %   \begin{tikzcd}[sep=large,ampersand replacement=\&]
    %     {} \& D\arrow[dl,swap,"q_1"]\arrow[dr,"q_2"]
    %           \arrow[d,dashed,"u"] \& {} \\
...
The following definition isn't necessary for understanding later sections
and chapters, but provides an interesting categorical framework for
understanding the λ-calculus. 

\begin{definition}\label[definition]{def:exponentials}
  If $\bfC$ has binary products, an \define{exponential} for objects
  $A,B∈\Obj\bfC$ consists of an object $B^A$ together an arrow $\eval:B^A×A→B$
  satisfying the following universal property: for any object $C∈\Obj\bfC$ and
  arrow $f:C×A→B$, there is an arrow $λf:C→B^A$ making the following diagram
  commute:
  \begin{center}
    \begin{tikzcd}[sep=large]
      C×A \arrow[dr, "f"] \arrow[d, swap, "λf×\id_A"] & {} \\
      B^A×A \arrow[r, swap, "\eval"]      & B
    \end{tikzcd}
  \end{center}
	
\end{definition}

\begin{definition}\label[definition]{def:cartesian-closed}
  A category is \define{Cartesian closed} when it has
  \begin{itemize}
    \itemsep-0.2em
  ...
  \end{itemize}
  ``CCC'' abbreviates ``Caratesian closed category''.
\end{definition}

\begin{remark}\label[remark]{rmk:lambek}
	We now have the vocabulary to (informally) extend the correspondence of
  \cref{sec:propositions-and-types} to statements about a certain kind of
  category.
  \cite{lambek}
  \TODO{}
\end{remark}
