...
\begin{theorem}[Fiber-initiality]\label[definition]{def:fiber-initiality}
  TODO: this is a definition
...
\begin{proof}
  ...
  \begin{center}
    \begin{tikzcd}
      Pℕ  \arrow[d, "η"] \arrow[r, "P^*g"] & PX \arrow[d, "α"] \\
      ℕ \arrow[r, "g"] & X
    \end{tikzcd}
  \end{center}
  ...
  If $b\≡\btrue$, then $h:\𝟙 → ℕ$. By the induction principle for
  ...
  \TODO{TODO: fix this with definition of f!}
  \begin{align*}
    \apply{(α∘(P^*g))}{\dpair{\btrue}{h}}
    &\jdeq \apply{α}{\dpair{\btrue}{g ∘ h}} \\
    &= \apply{α}{\dpair{\btrue}{g ∘ (\appply{\rec_{\𝟙}}{ℕ}{(\apply{h}{\unitelem})})}} \\
    &= \apply{α}{\dpair{\btrue}{(\λ{x}{\appply{\rec_{\𝟙}}{X}{(\apply{g}{(\apply{h}{x})}})})}}
    &&\quad\text{Function extensionality} \\
    &\jdeq \apply{f}{(\apply{g}{(\apply{h}{\unitelem})})} \\
    &\jdeq \apply{g}{(\apply{\suc}{(\apply{h}{\unitelem})})} 
    &&\quad\text{Definition of }g \\
    &\jdeq \apply{(g∘η)}{\dpair{\btrue}{h}}
...
\begin{lemma}\label[lemma]{lemma:nat-alg-mor-fun-unique}
  ...
  $\propeq{}{τ}{g}$, with $g$ is as in the previous proof.
\end{lemma}
\begin{proof}
...
\begin{proof}
  ...
  % By the induction principle for
\end{proof}
