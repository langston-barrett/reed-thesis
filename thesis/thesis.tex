...
theory. We first develop the classical Curry-Howard correspondence between (the
constructive subset of) intuitionistic natural dedication and the λ-calculus. We
proceed to explicate Martin-L\"of's theory of dependent types and the central
modern development in type theory: Vladimir Voevodsky's Univalence principle. We
go on to examine the nature of coinduction within this theory, presenting a
...
For a more comprehensive overview of \abbreviation{LC} and \abbreviation{STLC},
see \cite{barendregt}, which is the source of theorems and information covered
here.
...
\begin{example*}\label[example]{ex:calculations-in-lc}
  ...
  \begin{align*}
    ...
    &&\quad\text{(substitution)}.
  \end{align*}
  (Recall the meta-theoretical notation $[\mvar{b} \≔ \mvar{c}]$,
  \cref{notation:substitution}.)
...
\begin{center}
  \begin{tikzpicture}[node distance=1.5cm]
    ...
    \draw (H) edge [->] node[right] {\scriptsize β} (I);
    \draw (I) edge [->] node[below] {\scriptsize β} (Z);
  \end{tikzpicture}
\end{center}
...
Church-Rosser: every term has at most one β-normal form. The converse does not
hold.

\begin{example*}\label[example]{ex:non-terminating}
  Consider the term $⊥\≔ \λ{v}{\apply{v}{v}}$. Using the computation rule,
  \begin{align*}
    \apply{⊥}{⊥}
    &\≡ \apply{(\λ{v}{\apply{v}{v}})}{⊥}
    &&\quad\text{(Definition)} \\
    ...
    &&\quad\text{(substitution)} \\
    &\≡ \apply{⊥}{⊥}
    &&\quad\text{(β, substitution)} \\
    &\≡ \apply{⊥}{⊥}
    &&\quad\text{(β, substitution)} \\
    &\≡ ⋯
  \end{align*}
  It is clear that we could iterate this process.
  Now consider $⊥'\≔ \λ{v}{\appply{v}{v}{v}}$. Then
  \begin{align*}
    \apply{⊥'}{⊥'}
  ...
  ``compute'' ($⊥$), but we can build terms that actually \textit{expand} under
  rules meant to \textit{reduce} them, that are judgmentally equal to infinitely
  many other terms!
\end{example*}

While an ability to loop indefinitely is essential to Turing-Completeness, we
...
\begin{align*}
  ...
  {\prftree[r]{}
    ...
    {Γ ⊢ \type{\mvar{s}→\mvar{t}}}}
\end{align*}
The $\groundtype_i$ are called \define{ground types}.\footnote{There can be as
  many ground types as necessary or only one, it doesn't fundamentally change
  the character of the resulting theory.} 
...
} An elimination rule to balances this introduction:
\begin{align*}
  {\prftree[r]{}
...
unicity rules remain much the same, only now with some restrictions on their
domains:
\begin{align*}
  {\prftree[r]{\footnotesize β}
    {Γ, \mvar{a}:\mvar{s} ⊢ \mvar{b}:\mvar{t}}
...
\begin{example}\label[example]{ex:nat-functor-utt}
...
\begin{lemma}\label[lemma]{lemma:nat-alg-simpl}
...
\begin{lemma}\label[lemma]{lemma:nat-alg-mor}
  Let $(X,α)$ and $(Y,β)$ be $P$-algebras. Any function $f:X → Y$
...
\begin{lemma}\label[lemma]{lemma:nat-alg-mor-fun-unique}
...
\begin{lemma}\label[lemma]{lemma:nat-fib-alg-simpl}
  A fibered algebra structure (over $\ℕ$) on a family 
  $X:\ℕ\to\universe$ is given by a point $x_0:X_0$
  and a family of functions $f:\pit{n:\ℕ}{X_n\to X_{n+1}}$. Specifically, there
  is an equivalence
  \begin{equation*}
    \weq{\Fibalgtype(\dpair{ℕ}{\suc})}
        {\∑{X:\universe}X_0×\pit{n:\ℕ}{X_n\to X_{n+1}}}
  \end{equation*}
\end{lemma}

\begin{lemma}\label[lemma]{lemma:fiber-preinitiality-nat}
