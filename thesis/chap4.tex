...
\begin{lemma}\label[lemma]{lemma:polynomial-functor-transport}
  ...
  input and applying $g$.\footnote{This is a corollary of Lemma 2.9.6 in
    \cite{book}, which we prove as \coqname{M.Uniqueness.transport\_fun}. The
...
(all to be defined in this section) is contractible. In particular, The
fiber-initial property of $\ℕ$ (see \cref{def:fiber-initiality} and
\cref{lemma:fiber-initiality-nat}) is essential to proving \cref{lemma:cochains}.
We don't fully prove this result, but instead hope to provide some intuition
about what it means that $\N$ is a fiber-initial algebra.
...
\begin{lemma}[\coqname{M.Chains.standard\_limit\_of\_cochains}]
