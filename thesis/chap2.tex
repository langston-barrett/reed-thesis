...
\begin{equation*}
  \prftree[r]{}
    ...
    {Γ ⊢ \apply{S}{n}:\ℕ}
\end{equation*}
($S$ is what a logician might call a ``function symbol''). We
might define a type family
\begin{alignat*}{3}
  &\ttfun{Even} &&: \ℕ → \universei{0} \\
  &\apply{\ttfun{Even}}{0} &&\defeq \unittype  \\
  &\apply{\ttfun{Even}}{(\apply{S}{0})} &&\defeq \emptytype  \\
  &\apply{\ttfun{Even}}{(\apply{S}{(\apply{S}{n})})} &&\defeq \apply{\ttfun{Even}}{n}
\end{alignat*}
